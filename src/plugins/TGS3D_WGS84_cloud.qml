...
Item {
  ...
  // ---------- Settings ----------
  property int repeatSeconds: 2   // default (Breach) alarms repeat every N seconds while breaching
  property int warningInitialDuration: 12  // WARNING alarms start with same frequency as breach for this long
  property int warningRepeatSeconds: 8  // WARNING alarms repeat less frequently than breach after initial phase
  property int infoRepeatSeconds: 30  // INFO alarms repeat less frequently
  property int disableAlarmDuration: 300 // if the user acknowledges a breach,and triggers the manual off button disable the alarm sounds for N seconds (but keep showing the overlay) 
  ...
  property bool alarmSoundsDisabled: false
  property bool manualAlarmActive: false
  ...
  function checkAndHandleTrespass() {
    if (root.manualAlarmActive) return
  ...
  Timer {
    id: alarmSoundDisableTimer
    interval: root.disableAlarmDuration * 1000
    running: false
    repeat: false
    onTriggered: {
      root.alarmSoundsDisabled = false
      if (root.breaching) {
        var snd = root.alarmSoundFor(root.alarmType)
        if (snd) snd.play()
        root.toast("Alarm sounds resumed")
      }
    }
  }

  Timer {
    id: warningPhaseTimer
    interval: root.warningInitialDuration * 1000
    running: false
    repeat: false
    onTriggered: {
      if (root.breaching && root.isWarningType) {
        repeatTimer.interval = root.warningRepeatSeconds * 1000
      }
    }
  }

  function startBreach(areaName, type, isManual) {
    root.clearPendingDowngrade()
    root.breaching = true
    root.breachAreaName = areaName
    root.alarmType = type ? String(type) : ""
    root.manualAlarmActive = !!isManual
    root.breachAcknowledged = false
    root.alarmSoundsDisabled = false
    alarmSoundDisableTimer.stop()
    warningPhaseTimer.stop()

    // Set interval based on alarm type
    if (root.isInfoType) {
      repeatTimer.interval = root.infoRepeatSeconds * 1000
    } else if (root.isWarningType) {
      // WARNING starts at breach cadence, then slows down after initial duration.
      repeatTimer.interval = root.repeatSeconds * 1000
      if (root.warningInitialDuration > 0) {
        warningPhaseTimer.start()
      } else {
        repeatTimer.interval = root.warningRepeatSeconds * 1000
      }
    } else {
  ...
  function disableAlarmSoundsTemporarily() {
    if (!root.breaching) {
      root.toast("No active alarm to silence")
      return
    }

    if (!root.breachAcknowledged) {
      root.toast("Acknowledge breach first", "warning")
      return
    }

    if (root.disableAlarmDuration <= 0) {
      root.toast("Disable duration must be greater than 0", "warning")
      return
    }

    root.alarmSoundsDisabled = true
    alarmSoundDisableTimer.stop()
    alarmSoundDisableTimer.start()
    root.toast("Alarm sounds disabled for " + root.disableAlarmDuration + "s")
  }

  function formatDurationForButton(totalSeconds) {
  ...
  function runAlarmTest(type) {
    var t = normalizeAlarmType(type)
    root.startBreach("Manual alarm test", t, true)
  ...
  Rectangle {
    ...
    Column {
      ...
      Rectangle {
        ...
        height: 34
        radius: 8
        color: (root.breaching && root.breachAcknowledged) ? "#8e24aa" : "#616161"
        Text { anchors.centerIn: parent; text: "Silence " + root.formatDurationForButton(root.disableAlarmDuration); font.pixelSize: 14 }
        MouseArea {
          anchors.fill: parent
          onClicked: {
            root.disableAlarmSoundsTemporarily()
